...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      logger.info(
        `Fetched all availabilities, count: ${availabilities.length}`,
      );
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
      ...
      logger.info(
        `Fetched availability for employee ${employeeId}, count: ${availability.length}`,
      );
  ...
  availabilities: z
    .array(
      z.object({
        ...
        status: z
          .enum(["AVAILABLE", "UNAVAILABLE", "PREFERRED_TO_WORK"])
          .default("AVAILABLE"),
      }),
    )
...
router.put(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      logger.info(
        `Upserted ${results.length} availability entries for employee ${employeeId}`,
      ); // Log the number of entries upserted
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
      ...
      logger.info(
        `Created availability for user ${parsed.data.userId} on ${parsed.data.date} (${parsed.data.shift} - ${parsed.data.status})`,
      ); // Log the created availability
      res.status(201).json(availability);
    } catch (error: any) {
      if (error.code === "P2002") {
        return next(
          new AppError(
            "Availability already exists for this user, date and shift.",
            409,
          ),
        );
      }
...
router.delete(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to delete availability or record not found" });
      if (error.code === "P2025") {
...
router.patch(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      logger.info(
        `Updated approval status for availability ${id} to ${approvalStatus}`,
      ); // Log the approval status update
      res.json(updated);
    } catch (error) {
